...
class Entity(object):
    ...
    def give_birth(self):
        ...
        if child.sex == "M":
            f = open("../data/txtfiles/namelist_male.txt", 'r')
            lines = f.readlines()
            ...
            f = open("../data/txtfiles/namelist_female.txt", 'r')
            lines = f.readlines()
            first_name = np.random.choice(lines)[:-1]
            if np.random.random() < 0.25:
                middle_name = np.random.choice(lines)[:-1]
    ...
    def spawn_randomly(simu, name=None):
        ...
        if name == None:
            ...
            f = open("../data/txtfiles/familynames.txt", 'r')
    ...
    def spawn_here(simu, x, y, name=None):
        if name == None:
            f = open("../data/txtfiles/namelist.txt", 'r')
